...
class FunctionPacker {
    constructor(options) {
        ...
        this.funcPath = functionPath ? functionPath : path_1.default.resolve(path_1.default.join(root, name));
        this.tmpPath = root
            ? path_1.default.join(root, '.cloudbase_tmp')
            : path_1.default.join(process.cwd(), '.cloudbase_tmp');
    }
    async compressFiles() {
    ...
    getJavaFile() {
        ...
        const jarExist = utils_1.checkFullAccess(`${funcPath}.jar`);
        const zipExist = utils_1.checkFullAccess(`${funcPath}.zip`);
    ...
    async clean() {
        ...
        this.zipFilePath && del_1.default.sync([this.zipFilePath], { force: true });
