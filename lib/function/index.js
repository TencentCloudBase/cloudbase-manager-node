...
class FunctionService {
    ...
    async createFunction(func, functionRootPath, force = false, base64Code) {
        ...
        if (config.runtime && !validRuntime.includes(config.runtime)) {
            throw new error_1.CloudBaseError(`${funcName} Invalid runtime value：${config.runtime}. Now only support: ${validRuntime.join(', ')}`);
        }
     ...
     */
    async getFunctionDownloadUrl(functionName) {
        const { namespace } = this.getFunctionConfig();
        try {
            const { Url, CodeSha256, RequestId } = await this.scfService.request('GetFunctionAddress', {
