...
class StorageService {
    ...
    async uploadDirectory(options) {
        const { localPath, cloudPath = '', ignore, onProgress, onFileFinish, parallel, retry } = options;
        // 此处不检查路径是否存在
        // 绝对路径 /var/blog/xxxx
        const { bucket, region } = this.getStorageConfig();
    ...
    async uploadDirectoryCustom(options) {
        const { localPath, cloudPath, bucket, region, onProgress, onFileFinish, ignore, fileId = true, parallel = 20, retry = false } = options;
        // 此处不检查路径是否存在
        // 绝对路径 /var/blog/xxxx
        ...
        const failedFiles = [];
        await uploadFiles(Object.assign(Object.assign({}, params), { onFileFinish: (...args) => {
                const error = args[0];
                if (error && retry) {
                    uploadError = error;
        ...
        if (retry && uploadError) {
            return uploadFiles(Object.assign(Object.assign({}, params), { files: files.filter(file => failedFiles.includes(file.Key)), onFileFinish: (...args) => {
                    originFileFinish.apply(null, args);
                } }));
        }
    }
