...
class StorageService {
    ...
    async uploadFile(localPath, cloudPath) {
        ...
        const filePath = path_1.default.resolve(localPath);
        if (fs_1.default.existsSync(filePath) && !fs_1.default.statSync(filePath).isDirectory()) {
            localFilePath = path_1.default.resolve(localPath);
        }
        // 将 cloudPath 作为路径寻找
        const cloudAsLocalFilePath = path_1.default.resolve(cloudPath);
        if (fs_1.default.existsSync(cloudAsLocalFilePath) &&
            !fs_1.default.statSync(cloudAsLocalFilePath).isDirectory()) {
            localFilePath = path_1.default.resolve(cloudPath);
        }
        if (!localFilePath) {
    ...
    async downloadDirectory(cloudDirectory, localPath) {
        ...
        try {
            ...
            if (e.statusCode === 404) {
                throw new error_1.CloudBaseError(`云端路径不存在：${cloudDirectory}`);
            }
        }
