...
class StorageService {
     ...
     */
    async uploadFile(localPath, cloudPath = '', onProgress) {
     ...
     * @param {string} region
     */
    async uploadFileCustom(localPath, cloudPath, bucket, region, options = {}) {
        ...
        if (!fs_1.default.existsSync(resolveLocalPath)) {
            throw new error_1.CloudBaseError(`本地路径不存在：${resolveLocalPath}`);
        }
        ...
        if (fileStats.isDirectory()) {
            ...
            if (fs_1.default.existsSync(attemptFilePath)) {
     ...
     * @returns {Promise<void>}
     */
    async uploadDirectory(source, cloudDirectory = '', options = {}) {
     ...
     * @returns {Promise<void>}
     */
    async uploadDirectoryCustom(source, cloudDirectory, bucket, region, options = {}) {
        const { onProgress, onFileFinish, ignore, fileId = true } = options;
        // 此处不检查路径是否存在
        // 绝对路径 /var/blog/xxxx
        const resolvePath = path_1.default.resolve(source);
        // 在路径结尾加上 '/'
        const localPath = path_1.default.join(resolvePath, path_1.default.sep);
        ...
        const fileStatsList = filePaths.map(filePath => {
            ...
            let cloudPath = path_1.default.join(cloudDirectory, fileKeyPath).replace(/\\/g, '/');
     ...
     * @param {string} region
     */
    async createCloudDirectroyCustom(cloudPath, bucket, region) {
        const cos = this.getCos();
        const putObject = util_1.default.promisify(cos.putObject).bind(cos);
    ...
    async deleteFileCustom(cloudPathList, bucket, region) {
        ...
        const promises = cloudPathList.map(async (file) => await deleteObject({
            Bucket: bucket,
            Region: region,
            Key: file
        }));
     ...
     * @returns {Promise<void>}
     */
    async deleteDirectoryCustom(cloudDirectory, bucket, region) {
     ...
     * @returns {Promise<IListFileInfo[]>}
     */
    async walkCloudDirCustom(prefix, bucket, region, marker) {
        let fileList = [];
        const cos = this.getCos();
