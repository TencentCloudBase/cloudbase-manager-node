...
database 实例可以对数据库进行管理，以下表或集合为相同概念。
...
const { database } = new CloudBase({
    ...
    envId: 'Your envId' // 云开发环境ID，可在腾讯云-云开发控制台获取
})
```
...
```javascript
updateCollection(collectionName: string, options: array): object
```
...
```javascript
describeCollection(collectionName: string): object
```
...
```javascript
listCollections(options: object): object
```
...
```javascript
checkIndexExists(collectionName: string, indexName: string): object
```
...
```javascript
export(collectionName: string, file: object, options: object): object
```
...
```javascript
migrateStatus(jobId: number): object
```
